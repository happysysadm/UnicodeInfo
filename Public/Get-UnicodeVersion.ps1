...
<#
.SYNOPSIS
    Get the Age property of a Unicode glyph.
.DESCRIPTION
    Get-UnicodeVersion is a PowerShell function that queries the Unicde DataBase (UDB) for information
    on the Age property of a given glyph. It requires an Internet connection to download the UDB.
.PARAMETER Glyph
    A Unicode glyph.
.EXAMPLE
     Get-UnicodeVersion -Glyph 'Ω'
.EXAMPLE
     '£' | Get-UnicodeVersion
.NOTES
...
if(!$ageinfo) { 
    ...
    $sourceage = "http://www.unicode.org/Public/UNIDATA/DerivedAge.txt"

    $ageweb = Invoke-WebRequest $sourceage -UseBasicParsing

    $global:ageinfo = ($ageweb.content.split("`n").trim() -ne "") | sls "^#" -n | convertfrom-csv -Delimiter ';' -header "range","unicode version"

    }
    
$decimal = [int][char]$glyph

foreach($line in $ageinfo){
